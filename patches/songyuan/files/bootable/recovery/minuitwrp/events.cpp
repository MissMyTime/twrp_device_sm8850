...
static int vibrate_qcom_haptics(int timeout_ms)
{
	if (timeout_ms <= 0)
		return 0;
#ifdef USE_INPUT_FF_HAPTICS_K100PM
	if (timeout_ms > 120)
		timeout_ms = 120;
#else
	if (timeout_ms > 1000)
...
static int vibrate_input_ff(int timeout_ms)
{
...
#ifdef USE_INPUT_FF_HAPTICS_K100PM
		// The stock songyuan driver treats custom_len as a byte count and
		// silently accepts an empty effect when it receives the standard
		// three-element value.  Keep six zero-padded words so the input core
		// can safely copy the six units required by this vendor ABI.
		static int16_t custom_data[] = { 0, 0, 0, 0, 0, 0 };
#else
...
#ifdef USE_INPUT_FF_HAPTICS_K100PM
			effect->u.periodic.custom_len = 6;
#else
	...
	if (ff_fd >= 0) {
	#ifdef USE_INPUT_FF_HAPTICS_K100PM
		// The K100PM driver only really stops an effect from its erase
		// callback.  Do not carry a slot across requests or screens.
		if (ff_effect_id >= 0)
	...
	#ifdef USE_INPUT_FF_HAPTICS_K100PM
			// qcom-hv-haptics ignores EV_FF value 0 for the actual hardware
			// stop.  Match the verified vendor lifecycle: wait for this pulse,
			// send the input stop event, then erase the owning slot so the
			// driver's erase callback disables play before the next request.
			usleep(timeout_ms * 1000);
			play.value = 0;
			write(fd, &play, sizeof(play));
			int erase_ret = ioctl(fd, EVIOCRMFF, effect.id);
			LOGI("Input FF haptics K100PM cleanup slot %d: %s\n", effect.id,
			     erase_ret == 0 ? "ok" : strerror(errno));
			close(fd);
	#else
			ff_fd = fd;
			ff_effect_id = effect.id;
...
int vibrate(int timeout_ms)
{
    ...
    if (vib != nullptr) {
	    ...
	    // Some Qualcomm drivers accept qcom-haptics sysfs writes without
	    // producing an effect. Devices that explicitly select input FF use the
	    // verified qcom-hv-haptics event device first and recreate the effect on
	    // every request.
