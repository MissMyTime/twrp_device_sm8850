...
void TWPartitionManager::Decrypt_Data() {
	#ifdef TW_INCLUDE_CRYPTO
	static thread_local bool decrypt_data_in_progress = false;
	if (decrypt_data_in_progress) {
		LOGINFO("Metadata decrypt setup is already running; skipping recursive retry\n");
		return;
	}
	decrypt_data_in_progress = true;
	...
	if (Decrypt_Data && ((Decrypt_Data->Is_Encrypted && !Decrypt_Data->Is_Decrypted) || force_metadata_setup)) {
		...
		if (Decrypt_Data->Is_FBE) {
			...
			} else if (android::base::GetBoolProperty(
					"twrp.recovery.skip_default_fbe_password", false)) {
				LOGINFO("Skipping recovery default-password FBE attempt; waiting for the detected user credential\n");
...
//			}
	...
	decrypt_data_in_progress = false;
#endif
}
...
bool TWPartitionManager::Enable_MTP(void) {
#ifdef TW_HAS_MTP
	// Fastbootd and MTP are mutually exclusive FunctionFS transports. Never
	// let a delayed theme action replace a working Fastboot gadget with the
	// recovery MTP/ADB composite.
	if (android::base::GetBoolProperty(TW_FASTBOOT_MODE_PROP, false)) {
		LOGINFO("MTP: skipped in Fastbootd mode\n");
	...
	bool switch_usb_config = strcmp(old_value, "songyuan_mtp_adb") != 0;
	...
	if (mtppid) {
		...
		if (switch_usb_config) {
			...
			// Re-check after the FunctionFS wait: Fastbootd may have been selected
			// while this background worker was waiting for MTP readiness.
			if (android::base::GetBoolProperty(TW_FASTBOOT_MODE_PROP, false)) {
				LOGINFO("MTP: Fastbootd became active while waiting; aborting USB switch\n");
				Disable_MTP();
				return false;
			}
			char controller[PROPERTY_VALUE_MAX];
			...
			std::string cmd =
				...
				"setprop sys.usb.config songyuan_mtp_adb; "
				"echo none > /config/usb_gadget/g1/UDC 2>/dev/null || true; "
				"rm -f /config/usb_gadget/g1/configs/b.1/function0 "
				"/config/usb_gadget/g1/configs/b.1/function1 "
				"/config/usb_gadget/g1/configs/b.1/f1 "
