...
ifeq ($(TW_INCLUDE_BOOTSTRAP_LINKER), true)
  ifneq ($(filter arm64 x86_64 mips64,$(TARGET_ARCH)),)
    RECOVERY_LIBRARY_SOURCE_FILES += $(TARGET_RECOVERY_ROOT_OUT)/system/lib64/libvintf.so
  else
    RECOVERY_LIBRARY_SOURCE_FILES += $(TARGET_RECOVERY_ROOT_OUT)/system/lib/libvintf.so
  endif
else
...
LOCAL_POST_INSTALL_CMD += ; \
	mkdir -p $(TARGET_RECOVERY_ROOT_OUT)/system/bin/bootstrap \
		$(TARGET_RECOVERY_ROOT_OUT)/system/lib64/bootstrap; \
	cp -f $(TARGET_RECOVERY_ROOT_OUT)/system/bin/linker64 \
		$(TARGET_RECOVERY_ROOT_OUT)/system/bin/bootstrap/linker64; \
	cp -f $(TARGET_OUT_SHARED_LIBRARIES)/bootstrap/libc.so \
		$(TARGET_OUT_SHARED_LIBRARIES)/bootstrap/libm.so \
		$(TARGET_OUT_SHARED_LIBRARIES)/bootstrap/libdl.so \
		$(TARGET_OUT_SHARED_LIBRARIES)/bootstrap/libdl_android.so \
		$(TARGET_RECOVERY_ROOT_OUT)/system/lib64/bootstrap/
endif
