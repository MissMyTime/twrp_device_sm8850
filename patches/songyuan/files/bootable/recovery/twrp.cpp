...
static void Decrypt_Page(bool SkipDecryption, bool datamedia) {
	...
	if (prepare_fbe) {
		// Metadata-encrypted devices can reach this point before the generic TWRP
		// encryption flags and credential type have been populated.
	...
	if (DataManager::GetIntValue(TW_IS_ENCRYPTED) != 0) {
		if (SkipDecryption) {
			...
			// TW_NO_AUTO_DECRYPT still blocks the unsafe early default-password
			// attempt in partition setup. This controlled GUI-stage preparation is
			// needed to read locksettings.db and select pattern/PIN/password.
			...
			if (DataManager::GetIntValue(TW_IS_ENCRYPTED) != 0 &&
			    DataManager::GetIntValue(TW_CRYPTO_PWTYPE) != 0) {
				LOGINFO("Is encrypted, opening the detected credential page\n");
				if (DataManager::GetIntValue(TW_IS_FBE))
					DataManager::SetValue("tw_crypto_user_id", "0");
				if (gui_startPage("decrypt", 1, 1) != 0)
					LOGERR("Failed to start decrypt GUI page.\n");
			} else {
				LOGINFO("Credential type is still unavailable; continuing to the main page\n");
				PartitionManager.Update_System_Details();
			}
		}
	} else if (datamedia) {
...
int main(int argc, char **argv) {
	...
	// This is the current boot mode, unlike tw_enable_fastboot which only
	// describes the button state/capability. Themes use it to keep recovery
	// services such as MTP out of userspace Fastboot.
	DataManager::SetValue("tw_is_fastbootd", startup.Get_Fastboot_Mode() ? 1 : 0);
	printf("=> Linking mtab\n");
