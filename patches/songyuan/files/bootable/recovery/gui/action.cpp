	...
	if (simulate) {
		...
		if (auto_reflash) {
			...
			ret_val = TWinstall_zip(filename.c_str(), wipe_cache, (bool) !DataManager::GetIntValue(TW_SKIP_DIGEST_CHECK_ZIP_VAR));
				...
				if ((requested_slot == "A" || requested_slot == "B") && ret_val == 0) {
				...
				} else if (requested_slot == "A" || requested_slot == "B") {
					LOGINFO("Ignoring requested active slot %s because installer returned %d\n",
						requested_slot.c_str(), ret_val);
				}
		...
		if (should_reflash) {
			...
			if (TWFunc::Path_Exists("/tmp/recovery_backup.img")) {
				...
				if (ret_a == 0 && ret_b == 0) {
					gui_print("Finished flashing TWRP to both slots\n");
				} else {
					gui_msg(Msg(msg::kError, "reflash_twrp_err=Failed to reflash TWRP to slot A or B."));
				}
				unlink("/tmp/recovery_backup.img");
			} else {
...
int GUIAction::decrypt(std::string arg __unused)
{
	...
	if (simulate) {
		...
		// Decrypt_CE_storage already waits for the wrapped CE key before it
		// prepares user storage.  Do not add a second one-second delay after a
		// successful songyuan FBE operation; it only holds the action page and
		// delays the navigation bar becoming interactive.
		char product_device[PROPERTY_VALUE_MAX];
		property_get("ro.product.device", product_device, "");
		if (strcmp(product_device, "songyuan") != 0)
			::sleep(1);
		} else {  // for FDE
...
static pthread_mutex_t mtp_background_lock = PTHREAD_MUTEX_INITIALIZER;
static bool mtp_background_running = false;

static void* start_mtp_background_worker(void*)
{
	// A theme condition may become true while the decrypt action is still
	// returning. Wait outside the render thread so main-page input remains
	// responsive and no second ActionThread is started on top of decrypt.
	for (int retry = 0; retry < 200 &&
		 DataManager::GetIntValue(TW_ACTION_BUSY); ++retry) {
		usleep(50000);
	}

	if (DataManager::GetIntValue(TW_ACTION_BUSY)) {
		LOGINFO("MTP background start skipped because the current action stayed busy\n");
	} else if (android::base::GetBoolProperty(TW_FASTBOOT_MODE_PROP, false) ||
		   DataManager::GetIntValue("tw_is_fastbootd")) {
		LOGINFO("MTP background start skipped in Fastbootd mode\n");
	} else if (!DataManager::GetIntValue("tw_mtp_enabled")) {
		LOGINFO("MTP background worker: starting after the current action completed\n");
		PartitionManager.Enable_MTP();
	}

	pthread_mutex_lock(&mtp_background_lock);
	mtp_background_running = false;
	pthread_mutex_unlock(&mtp_background_lock);
	return nullptr;
}

int GUIAction::startmtpbackground(std::string arg __unused)
{
	pthread_mutex_lock(&mtp_background_lock);
	if (mtp_background_running) {
		pthread_mutex_unlock(&mtp_background_lock);
		return 0;
	}
	mtp_background_running = true;
	pthread_mutex_unlock(&mtp_background_lock);

	pthread_t worker;
	if (pthread_create(&worker, nullptr, start_mtp_background_worker, nullptr) != 0) {
		LOGERR("Unable to create MTP background worker\n");
		pthread_mutex_lock(&mtp_background_lock);
		mtp_background_running = false;
		pthread_mutex_unlock(&mtp_background_lock);
		return -1;
...
int GUIAction::enablefastboot(std::string arg __unused) {
	// MTP owns the same configfs gadget. Tear it down before selecting
	// FunctionFS fastboot, otherwise a late MTP worker can rebind the host as
	// a recovery/ADB device after Fastbootd is already on screen.
...
int GUIAction::wlanstart(string arg __unused) {
	...
	// Use system/bin copies whose executable modes are fixed by fs_config.
	// Files copied under /sbin are packed as 0644 on this ramdisk, and an init
	// chmod is denied by the compact recovery SELinux policy.
	int ret = TWFunc::Exec_Cmd("/system/bin/wifi-load-modules.sh");
	if (ret != 0) {
		if (logBox) logBox->AddLogLine("[ERROR] Failed to load WiFi modules", "error");
		LOGERR("Failed to load WiFi modules\n");
		return -1;
	}
	...
	ret = TWFunc::Exec_Cmd("LD_LIBRARY_PATH=/sbin/lib64:/system/lib64:/system_ext/lib64:/vendor/lib64 /system/bin/wpa_supplicant -B -iwlan0 -Dnl80211 -c/tmp/recovery/wpa_supplicant.conf -O/tmp/recovery/sockets");
	if (ret != 0) {
...
int GUIAction::wlanscan(std::string arg __unused) {
    ...
    // ========= 2. 执行 scan_results（带空结果重试） =========
    std::vector<std::string> lines;
    if (scan_ok) {
        // 扫描成功，等待驱动完成（首次扫描需要更长时间）
        usleep(2000 * 1000); // 等待 2 秒

        for (int results_retry = 0; results_retry < 2; ++results_retry) {
            lines.clear();
            ...
            // 第一次 scan_results 为空，再等 1.5 秒让驱动完成
            if (results_retry == 0) {
                logBox->AddLogLine("[INFO] Scan in progress, waiting...", "normal");
                gui_forceRender();
                usleep(1500 * 1000);
            }
        }
        ...
        logBox->AddLogLine("        Reply: " + last_reply, "error");
        gui_forceRender();
        // 尝试使用缓存的扫描结果兜底
...
int GUIAction::wlantest(std::string arg __unused) {
    ...
    // Recovery does not run Android's full netd resolver stack.  A DHCP DNS
    // configuration is still useful, but treating a libc hostname lookup as
    // the Internet test produces a false red error even when IP routing works.
    std::string dns = run_command_get_output(
        "awk '/^nameserver[ \\t]+/ {print $2; exit}' /etc/resolv.conf 2>/dev/null");
    while (!dns.empty() && (dns.back() == '\n' || dns.back() == '\r')) dns.pop_back();
    if (!dns.empty())
        logBox->AddLogLine("[INFO] DNS configured: " + dns, "normal");
    else
        logBox->AddLogLine("[INFO] DNS not supplied by DHCP", "normal");
    gui_forceRender();

    // Test routed Internet access with an IP literal so DNS availability is
    // not confused with Wi-Fi or gateway connectivity.
    std::string internet = "8.8.8.8";
