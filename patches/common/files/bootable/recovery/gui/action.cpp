...
int GUIAction::wlanstart(string arg __unused) {
	...
	// Set permissions
	TWFunc::Exec_Cmd("chmod 0755 /sbin/wifi-load-modules.sh /sbin/wifi-dhcp.sh /sbin/wpa_supplicant /sbin/wpa_cli 2>/dev/null");

	// Load WiFi modules
	int ret = TWFunc::Exec_Cmd("/sbin/wifi-load-modules.sh");
	if (ret != 0) {
